...
export interface FifaTeamProps {
    nationName: string,
...
export const FifaTeam: React.FunctionComponent<FifaTeamProps> = (props: FifaTeamProps) => {
  ...
  const teamRecord = useWikipediaByFootballTeam(props.nationName);

  const { teamNationalLogo, teamNationalFlag, teamInformation } = useWikipediaByCountry(props.nationName);
  ...
  //document.addEventListener('keypress', (e) => { e.charCode === 110 ? setSelectedOpponentIndex(selectedOpponentIndex + 1) : null });

    if (document.getElementsByClassName('jersey-container').item(0)) {
        if (document.getElementsByClassName('home-jersey').item(0)) {
            (document.getElementsByClassName('home-jersey').item(0) as Element).innerHTML = teamInformation.jersey[0];
        }
        if (document.getElementsByClassName('away-jersey').item(0)) {
            (document.getElementsByClassName('away-jersey').item(0) as Element).innerHTML = teamInformation.jersey[1];
        }
        if (document.getElementsByClassName('third-jersey').item(0)) {
            (document.getElementsByClassName('third-jersey').item(0) as Element).innerHTML = teamInformation.jersey[2];
        }
    }
                ...
                <div className='fifa-team-details'>
                  {
                    selectedOpponent.length === 0 && selectedOpponents.length === 0 ?
                      <div>
                        ...
                        <h2 style={{ fontSize: '60px' }}>{props.nationName}</h2>
                        <img src={teamNationalLogo?.toString()} />
                      </div> :
                      <div>
                        <img src={teamNationalLogo?.toString()} />
      ...
      }
          {
              !isAnimation && props.showAdvancedPanel && <div className='select-opponent-team'>
                  <button onClick={() => {
                      ...
                      disabled={props.nationName.length === 0}>
                  ...
                  <button onClick={() => {
                      ...
                      disabled={props.nationName.length === 0 || selectedOpponent.length === 0}>
                      {isAnimation ? 'Stop Animation' : 'Start Animation'}
                  </button>
                  <select name="teams" value={selectedOpponent} onChange={(e) => { setSelectedOpponent(e.target.value); }} disabled={props.nationName.length === 0}>
      ...
      }
          {
              showSelectedTeamDetails && <div className='selected-team-details'>
                  <div className='team-details-container-column-1'>
                      <img src={teamNationalFlag} />
                      <img src={teamNationalLogo as string} />
                  </div>
                  <div className='team-details-container-column-2'>
                      {
                          !showKits && <>
                              <div><span>Confederation </span><span>{teamInformation.confederation}</span></div>
                              <div><span>Head Coach </span><span>{teamInformation.headCoach}</span></div>
                              <div><span>Captain </span><span>{teamInformation.captain}</span></div>
                              <div><span>Most Caps </span><span>{teamInformation.mostCaps}</span></div>
                              <div><span>Top Scorer </span><span>{teamInformation.topScorer}</span></div>
                          </>
                      }
                      {
                          showKits && teamInformation.jersey.length > 0 &&
                          < div className='jersey-container'
                              style={teamInformation.jersey[2] ? { gridTemplateColumns: '1fr 1fr 1fr' } : { gridTemplateColumns: '1fr 1fr' }}>
                              {teamInformation.jersey[0] && <div className='home-jersey' style={teamInformation.jersey[2] ? { zoom: '280%' } : { zoom: '320%' }}>{'...Loading'}</div>}
                              {teamInformation.jersey[1] && <div className='away-jersey' style={teamInformation.jersey[2] ? { zoom: '280%' } : { zoom: '320%' }}>{'...Loading'}</div>}
                              {teamInformation.jersey[2] && <div className='third-jersey'>{'...Loading'}</div>}
                          </div>
                      }
                  </div>
              </div>
